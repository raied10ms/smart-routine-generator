...
const taskTypeColor: Record<string, string> = {
  CQ: "bg-red-100 text-red-700",
  MCQ: "bg-amber-100 text-amber-700",
  "গণিত": "bg-blue-100 text-blue-700",
  "রিভিশন": "bg-green-100 text-green-700",
};

function getTaskBadgeClass(taskType: string): string {
  for (const [key, cls] of Object.entries(taskTypeColor)) {
...
export default function RoutineTable({ days, staggered }: Props) {
  let currentPhase = 0;
      ...
      {days.map((day, dayIdx) => {
        ...
        return (
          <div key={day.dayNumber}>
            ...
            <div
              className={`bg-white rounded-[var(--radius-card)] shadow-[var(--shadow-card)] border border-[var(--color-border)] overflow-hidden ${
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[var(--color-surface)] border-b border-[var(--color-border)]">
                <div className="flex items-center gap-2">
                  ...
                  {day.isExtreme && (
                    <span className="text-[11px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">পাওয়ার ডে ⚡</span>
                  )}
                  {day.isWeekend && (
                    <span className="text-[11px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-medium">🌙 হালকা দিন</span>
                  )}
                </div>
                <span className="text-[13px] font-semibold text-[var(--color-primary)]">
                  মোট: ~{formatTime(day.totalTimeMin)}
                </span>
              </div>
              ...
              {(() => {
                ...
                return Object.entries(grouped).map(([subject, entries], groupIdx) => (
                  <div key={`${day.dayNumber}-${subject}`} className={groupIdx > 0 ? "border-t border-[var(--color-border)]" : ""}>
                    ...
                    {/* Chapter entries */}
                    {entries.map((entry, i) => (
                      <div key={`${day.dayNumber}-${subject}-${i}`}
                        className="flex items-center gap-3 px-4 pl-7 py-1.5">
                        <span className="flex-1 min-w-0 text-[12px] text-[var(--color-text-muted)] truncate">{entry.chapterName}</span>
                        <span className={`shrink-0 text-[11px] font-medium px-2 py-0.5 rounded-full ${getTaskBadgeClass(entry.taskType)}`}>
                          {entry.taskType}
                        </span>
                        <span className="shrink-0 text-[13px] font-medium w-12 text-right">
                          {toBanglaNum(entry.timeMin)} মি.
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              {day.entries.length === 0 && (
                <div className="px-4 py-3 text-center text-[13px] text-[var(--color-text-muted)]">
                  কোনো টাস্ক নেই
                </div>
              )}
            </div>
